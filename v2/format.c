...
int slider_set(double ratio, int fast)
{
	...
	args->seekfrom=args->videotimestamp;
...
void videoplayback_start(const char *fn, int has_video, int has_audio)
{
	...
	video_decode_args=(VideoDecodeArgs*)malloc(sizeof(VideoDecodeArgs));
	if(!video_decode_args)
	{
		LOG_ERROR("Alloc error");
		return;
	}
	memset(video_decode_args, 0, sizeof(*video_decode_args));
...
int load_media(const wchar_t *filename, Image16 **image, int erroronfail)
{
	...
	if(video_stream_index!=-1)
	{
		AVCodecContext *codecContext=avcodec.avcodec_alloc_context3(videocodec);
		if(!codecContext)
		{
			if(erroronfail)
				LOG_WARNING("Allocation error");
