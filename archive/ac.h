...
inline void abac_enc_v2(ABACEncContext *ctx, unsigned short p0, int bit)
{
	unsigned mid;
	int pLPS=p0<0x8000?p0:0x10000-p0;

	for(;;)//renorm
	{
		mid=(int)((unsigned long long)(ctx->hi-ctx->lo)*pLPS>>16);
		if(ctx->lo+3>ctx->lo&&ctx->lo+3<=ctx->hi&&mid>0)
			break;
		if(ctx->nbits>=32)
		{
...
inline int abac_dec_v2(ABACDecContext *ctx, unsigned short p0)
{
	unsigned mid;
	int pLPS=p0<0x8000?p0:0x10000-p0;
	for(;;)//renorm
