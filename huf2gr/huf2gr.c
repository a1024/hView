...
//#if 1
	const char *srcfn=
		"E:/Share Box/Scope/20241107/20241107_164228_958.huf"	//7408496/9987840		7408496/7990272 bytes
		;
	const char *dstfn=
		"D:/ML/mystery.gr"					//3852960/9987840		3970607/7990272 bytes
		;
#else
	if(argc!=3)
	{
	...
	t=time_sec()-t;
#ifdef LOUD
	int depth=FLOOR_LOG2(nlevels);
	int usize=(iw*ih*depth+7)>>3;
	int csize=0;
	{
		struct stat info={0};
		stat(srcfn, &info);
	...
	printf("HUF.D %10lf sec %12.6lf MB/s  %10d/%10d\n"
		, t
		, usize/(t*1024*1024)
		, csize
		, usize
...
//	pgm_save_clamp("20241109_181300_huf_rct.pgm", (short*)image, iw, ih, nlevels, yidx);
	...
	t=time_sec();
	gr2_save(dstfn, (short*)image, iw, ih, nlevels, bayermatrix);
	t=time_sec()-t;
#ifdef LOUD
	{
		struct stat info={0};
	...
	printf("GR .E %10lf sec %12.6lf MB/s  %10d/%10d\n"
		, t
		, usize/(t*1024*1024)
		, csize
		, usize
...
#ifdef _MSC_VER
	{
		...
		t=time_sec();
		im2=gr2_load(dstfn, &iw2, &ih2, &nlevels2, bayer2);
		t=time_sec()-t;
#ifdef LOUD
